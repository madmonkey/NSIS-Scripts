...
;-------------------------------- 
	...
	!define productversion "7.0.0.0"
	!define setupversion "7.0.0.0"
...
Install:
	...
	*/
	...
	ReadINIStr $0 "$EXEDIR\${SetupIni}" "Site" "UseSSL"
	${If} $0 == "0"
		${ReplaceToken} [SecurityMode] "None" "$INSTDIR\$sector"
		${ReplaceToken} [SecuredMetaData] "httpGetEnabled" "$INSTDIR\$sector"
	${Else}
		${ReplaceToken} [SecurityMode] "Transport" "$INSTDIR\$sector"
		${ReplaceToken} [SecuredMetaData] "httpsGetEnabled" "$INSTDIR\$sector"
	${EndIf}
	
	ReadINIStr $0 "$EXEDIR\${SetupIni}" "Application" "MinutesToCacheRoles"
	${ReplaceToken} [MinutesToCacheRoles] "$0" "$INSTDIR\$sector"
	
	ReadINIStr $0 "$EXEDIR\${SetupIni}" "Application" "IgnoreCaseForPassword"
	${ReplaceToken} [IgnoreCaseForPassword] "$0" "$INSTDIR\$sector"
	
	ReadINIStr $0 "$EXEDIR\${SetupIni}" "Database" "WebConnectionString"
	${ReplaceToken} [WebConnectionString] $0 "$INSTDIR\$sector"
	
	ReadINIStr $datasource "$EXEDIR\${SetupIni}" "Database" "Datasource"
	${If} $datasource == ""
		StrCpy $datasource "."
	${EndIf}
	${ReplaceToken} [Datasource] $datasource "$INSTDIR\$sector"
	
	ReadINIStr $database "$EXEDIR\${SetupIni}" "Database" "Database"
	${If} $database == ""
		StrCpy $database "PW"
	${EndIf}
	${ReplaceToken} [Database] $database "$INSTDIR\$sector"
	
	ReadINIStr $0 "$EXEDIR\${SetupIni}" "Database" "WebConnectionProvider"
	${ReplaceToken} [WebConnectionProvider] $0 "$INSTDIR\$sector"
	${ReplaceToken} [ApplicationDir] "$INSTDIR\$sector\" "$INSTDIR\$sector"
	...
	${If} $uname == "(SSPI)"
		...
		StrCpy $pword ""
	${EndIf}
	${ReplaceToken} [Uname] $uname "$INSTDIR\$sector"
	${ReplaceToken} [Pword] $pword "$INSTDIR\$sector"
